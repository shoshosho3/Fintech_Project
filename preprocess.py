...
def merge_start(row):
    ...
    while True:
        ...
        except KeyError:  #There is no value
            difference = (row['adjusted_datetime'] - row['date']).days
            if difference > 7:
                break
            row['adjusted_datetime'] += pd.DateOffset(days=1)
            specific_date = row['adjusted_datetime'].date().strftime('%Y-%m-%d')
            data = yf.download(row['stock'], start=specific_date,
                               end=pd.to_datetime(specific_date) + pd.DateOffset(days=1),
                               interval='1d', progress=False)
        except:
            return None
...
print(df_stock_temp)
df_stock_temp.to_excel('final_data.xlsx', index=False)
